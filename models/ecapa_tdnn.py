...
class ECAPA_TDNN(nn.Module):
    def __init__(self, in_channels=80, channels=512, embd_dim=192, class_num=42,encoder_type='SAP',n_mels=40, flag= True, log_input=True, **kwargs):
        super().__init__()
        ...
        self.fc1 = nn.Linear(embd_dim, class_num)
...
def MainModel(nOut=192, nClasses=1211, **kwargs):
    model = ECAPA_TDNN(80, 512, nOut, nClasses, **kwargs)
    print(nClasses)  
    return model

if __name__ == '__main__':
    x = torch.zeros(3,16000)
    model = ECAPA_TDNN(in_channels=80, channels=512, embd_dim=192, class_num=42,n_mels=80)
    out = model(x)
    print(model)
