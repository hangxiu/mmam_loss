...
parser.add_argument('--save_path', 	type=str, default="/data/jmh/data", help='Target directory');
...
def convert(args):
	...
	#files 	= glob.glob('%s/voxceleb2/dev/aac/*/*/*.m4a'%args.save_path)
	files 	= glob.glob('%s/voxceleb2/dev/opuswav/*/*/*.opus'%args.save_path)
	files.sort()
	print(files)
	print('Converting files from AAC to WAV')
	for fname in tqdm(files):
		#outfile = fname.replace('.m4a','.wav')
		#outfile = fname.replace('.wav','.opus')
		outfile = fname.replace('.opus','_opus.wav')
		#outfile = fname.replace('.wav','.aac')
		# outfile = outfile.replace('test','test_tem')
		...
		if os.path.exists(outfile):
			...
			out = subprocess.call('ffmpeg -y -i %s -ac 1 -vn -acodec pcm_s16le -ar 16000 %s >/dev/null 2>/dev/null' %(fname,outfile), shell=True)
			#out = subprocess.call('ffmpeg -y -i %s -ac 1 -vn -acodec libopus -ar 16000 %s >/dev/null 2>/dev/null' %(fname,outfile), shell=True)
			#out = subprocess.call('ffmpeg -y -i %s -ac 1 -acodec aac -strict -2 -ar 16000 %s >/dev/null 2>/dev/null' %(fname,outfile), shell=True)
		# ffmpeg -i 00001.wav -acodec aac -strict -2 -vn output.aac
		#ffmpeg -i input.wav -c:a libfdk_aac -b:a 128k output.m4a
		#ffmpeg -i input.wav -c：a libfdk_aac -profile：aac_he -b：64k output.m4a
		 #-codec:a aac output.aac
			if out != 0:
